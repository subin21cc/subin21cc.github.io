---
title: "[Capstone-Start] 프로젝트 소개 페이지 제작 및 배포"
date: 2026-03-23 12:00:00 +0900
categories: [Capstone, Start]
tags: [dev, capstone]
---

**📅 일시:** 2026. 03. 22

**💬 내용:** 프로젝트 소개 페이지 제작 및 Vercel 배포

---

### **1. 프로젝트 소개 페이지 제작**

우리 프로젝트의 비전과 주요 기능을 한눈에 확인할 수 있는 소개 페이지를 제작했다. 서비스의 핵심인 'AI 기반 초개인화'와 '통합 건강 관리'의 가치를 담아내는 데 주력했다.

### **2. Vercel을 활용한 배포 완료**

제작한 페이지를 팀원 및 외부와 원활하게 공유하기 위해 **Vercel**을 활용하여 배포를 완료했다. 아래 링크에서 확인 가능하다.

- 🔗 **배포 주소:** https://sudo-capstone-project.vercel.app/
![사진1](/assets/img/posts/capstone/capstone-start-page_distribution.png)


### **3. 향후 계획**

소개 페이지를 바탕으로 팀의 프로젝트 방향성을 상기하며, 실제 서비스 구현을 위한 상세 설계 및 개발 단계로 나아갈 예정이다.
