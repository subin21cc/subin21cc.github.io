...
발표가 코앞이라 오늘은 PR이 많이 쪼개졌다. 새벽에 README 브랜드 배너부터 시작해서, 낮에 배포 워크플로우 4번 손보고, 저녁에 카메라/모달/FAB 작업, 밤에 차트 Y축까지 정리. 총 **PR 9개**를 머지했다. 짧게 끊어 가는 게 시연 안정성에 더 안전하다는 판단이었다.

## 1. README 브랜드 배너 & 로고 (PR #14)

README 상단에 **브랜드 배너 + 로고**를 박았다. On-Care 타이틀 옆에 작은 로고를 둬서 발표 자료 캡처 시 시각적 무게중심이 잡히도록.
디자인 툴은 잘 몰라서 직접 그리느라 애를 좀 먹었다.
...
오늘은 PR 9개로 짧게 끊어 진행한 날이다. README 배너 →  GitHub Pages 배포 디버깅 4단계(워크플로우 / path / base-href / WASM) → 식단 카메라+분석 UI 셸 + FAB 통일 + 건강 지표 추이 모달 + 설정 모달 4종 → 차트 Y축 임상 범위 클램핑. 라이브 시연 URL이 정상 동작하고, 발표 시나리오의 모든 흐름이 매끄럽게 흘러갈 수 있는 상태가 됐다.
