...
테스트 작성에서 걸린 문제도 있었다. 동시성을 재현해야 하므로 테스트 트랜잭션 롤백을 쓸 수 없고(다른 스레드에서 픽스처가 안 보인다), 그러면 정리를 직접 해야 한다. 처음엔 정리를 네이티브 SQL로 했는데 `groups`가 예약어라 백틱 인용이 H2에서 다르게 해석돼 tearDown이 깨졌다. 정리가 안 되니 다음 테스트가 이메일 중복으로 연쇄 실패했다. JPA 삭제로 바꿔 해결했다.
